...
class TestRunAgentVerify(unittest.TestCase):
    ...
    def test_success_with_model(self):
        ...
        with patch(
            "arcval.connections.TextAgentConnection.verify",
...
class TestMainDispatch(unittest.TestCase):
    ...
    def test_llm_eval_only_no_config_exits(self):
        with self.assertRaises(SystemExit):
            self._run_with_argv(
                ["arcval", "llm", "--eval-only", "--dataset", "/tmp/x.json"]
            )
    ...
    def test_llm_eval_only_debug_forwards_flags(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            ds = Path(tmp) / "ds.json"
            ds.write_text("[]")
            with patch("arcval.llm.run_tests.main", AsyncMock(side_effect=_capture)):
                self._run_with_argv(
                    [
                        "arcval",
                        "llm",
    ...
    def test_llm_benchmark_debug_forwards_flags(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("arcval.llm.benchmark.main", AsyncMock(side_effect=_capture)):
                self._run_with_argv(
                    [
                        "arcval",
                        "llm",
    ...
    def test_simulations_voice_runs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cfg.write_text("{}")
            with patch(
                "arcval.agent.run_simulation.main", AsyncMock(return_value=None)
            ):
                self._run_with_argv(
                    [
                        "arcval",
