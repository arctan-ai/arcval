# arcval.connections module
"""
Connection types for injecting external agents into arcval evaluations.
...
class TextAgentConnection:
    ...
    Arcval sends a fixed request and expects a fixed response format.
    ...
    ── Response (agent must return) ────────────────────────────────────────
        ...
        call) is the one Arcval aggregates into the per-model mean cost — the
