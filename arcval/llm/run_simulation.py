...
from arcval.llm.metrics import evaluate_simuation, DEFAULT_SIMULATION_JUDGE_MODEL
...
def _resolve_simulation_parallel(cli_value: Optional[int] = None) -> int:
    """Resolve simulation concurrency: CLI flag > ARCVAL_SIMULATION_PARALLEL > default."""
    if cli_value is not None and cli_value > 0:
        return cli_value
    env_value = os.getenv("ARCVAL_SIMULATION_PARALLEL")
    if env_value:
        try:
...
async def run_simulation_with_agent(
    ...
    :meth:`~arcval.connections.TextAgentConnection.call`.
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of simulations to run in parallel (overrides ARCVAL_SIMULATION_PARALLEL)",
    )
    parser.add_argument(
        "--eval-only",
        action="store_true",
