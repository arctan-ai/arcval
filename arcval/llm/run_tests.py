...
if TYPE_CHECKING:
    from arcval.connections import TextAgentConnection  # noqa: F401
import os
...
from arcval.llm.metrics import test_response_llm_judge, evaluate_simuation
from arcval.llm._metrics_utils import _numeric_or_none, _latency_percentiles
...
def _resolve_test_parallel(cli_value: Optional[int] = None) -> int:
    """Resolve max test-case concurrency: CLI flag > ARCVAL_TEST_PARALLEL > default."""
    if cli_value is not None and cli_value > 0:
        return cli_value
    env_value = os.getenv("ARCVAL_TEST_PARALLEL")
    if env_value:
        try:
...
async def run_test_external(
    ...
    :meth:`~arcval.connections.TextAgentConnection.call` for details. It may
    ...
    Args:
        ...
        agent: A :class:`~arcval.connections.TextAgentConnection`.
...
async def main():
    ...
    For multi-model benchmark, use benchmark.py via `arcval llm -m model1 model2 ...`
    """
