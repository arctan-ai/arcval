...
Refer to the [sample dataset](https://github.com/ARTPARK-SAHAI-ORG/arcval/blob/main/examples/stt/sample_input) for a template.
...
Refer to the [sample config](https://github.com/ARTPARK-SAHAI-ORG/arcval/blob/main/examples/stt/config.json) for a template.
...
<AccordionGroup>
  <Accordion title="semantic_match (default evaluator system prompt)">
    Matches [`DEFAULT_STT_EVALUATOR`](https://github.com/ARTPARK-SAHAI-ORG/arcval/blob/main/arcval/judges.py) in `arcval/judges.py` when no `--config` is passed.
