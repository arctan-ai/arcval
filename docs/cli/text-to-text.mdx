...
<AccordionGroup>
  <Accordion title="correctness (default evaluator system prompt)">
    When you omit top-level `evaluators` and pass a string `criteria`, this is the implicit evaluator from [`DEFAULT_LLM_TEST_EVALUATOR`](https://github.com/ARTPARK-SAHAI-ORG/calibrate/blob/main/arcval/judges.py) that is used by default. Your `criteria` string is substituted for `{{criteria}}`.
