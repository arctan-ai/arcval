...
- **Arcval agent** — define your agent inside the config file and evaluate across models.
- **Agent connection** — connect your existing agent via HTTP and run tests against it directly.

### Arcval agent

Define your agent's system prompt and tools directly in the config file. Arcval runs the evaluation using an LLM of your choice.

Refer to [this sample](https://github.com/ARTPARK-SAHAI-ORG/arcval/blob/main/examples/llm/config-internal-agent.json) for a full template.
...
If you already have an agent running, you can connect arcval directly to it instead of redefining your agent's system prompt and tools.

Refer to [this sample](https://github.com/ARTPARK-SAHAI-ORG/arcval/blob/main/examples/llm/config-external-agent.json) for a full template.

#### Connect your agent

| Key             | Required | Description                                                 |
| --------------- | -------- | ----------------------------------------------------------- |
| `agent_url`     | Yes      | The URL arcval will make a POST request to               |
...
Arcval will make a POST request to your `agent_url` with the following body:
...
```bash
arcval llm --verify --agent-url https://your-agent.com/chat --agent-headers '{"Authorization": "Bearer YOUR_API_KEY"}'
```
...
See [`examples/llm/config-conversation.json`](https://github.com/ARTPARK-SAHAI-ORG/arcval/blob/main/examples/llm/config-conversation.json) for a full template.
...
<AccordionGroup>
  <Accordion title="correctness (default evaluator system prompt)">
    When you omit top-level `evaluators` and pass a string `criteria`, this is the implicit evaluator from [`DEFAULT_LLM_TEST_EVALUATOR`](https://github.com/ARTPARK-SAHAI-ORG/arcval/blob/main/arcval/judges.py) that is used by default. Your `criteria` string is substituted for `{{criteria}}`.
...
<AccordionGroup>
  <Accordion title="Arcval agent example">
    Same structure as [`examples/llm/config-internal-agent.json`](https://github.com/ARTPARK-SAHAI-ORG/arcval/blob/main/examples/llm/config-internal-agent.json), with documentation-only `//` comments before each test case (`jsonc` — strip comments if you paste into a strict JSON parser).
  ...
  <Accordion title="Agent connection example">
    Same structure as [`examples/llm/config-external-agent.json`](https://github.com/ARTPARK-SAHAI-ORG/arcval/blob/main/examples/llm/config-external-agent.json), with documentation-only `//` comments before each test case (`jsonc` — strip comments if you paste into a strict JSON parser).
...
Run `arcval llm` with no arguments to launch the interactive UI:

```bash
arcval llm
```

**Arcval agent** — the UI detects that your config defines a system prompt and tools and guides you through:
...
3. **Verify** — arcval verifies the connection; shows success or failure with the option to go back
...
3. **Model entry** — enter a model name; arcval verifies the connection with that model before asking if you want to add another
4. **Confirm models** — review the list and add more or proceed
5. **Output directory** — where results will be saved (defaults to `./out`)
...
```bash
arcval llm -c config.json -m openai/gpt-4.1 claude-sonnet-4-5 -p openrouter -o ./out
```
...
```bash
arcval llm -c config.json -o ./out
```

Arcval verifies the connection before running. Results are saved directly to the output directory.

Benchmark across models — arcval verifies the connection for each model, then runs the test suite once per model. The model name is included in each request so your agent can route to the right model:

```bash
arcval llm -c config.json -m gemma-4-26b-a4b-it gpt-4o -o ./out
```
...
```bash
arcval llm -c config.json -o ./out --skip-verify
```

```bash
arcval llm -c config.json -m gemma-4-26b-a4b-it gpt-4o -o ./out --skip-verify
```
