...
Refer to [this sample](https://github.com/ARTPARK-SAHAI-ORG/arcval/blob/main/examples/tts/sample.csv) for a template.
...
```bash
arcval tts -p openai google -i sample.csv -o ./out --config config.json
```
...
| `evaluators[].judge_model`   | string | OpenRouter model id for this evaluator (default: `openai/gpt-audio`). Must be an audio-capable model in the [OpenRouter catalog](https://openrouter.ai/models) — for example OpenAI’s audio models or Google’s Gemini audio-capable entries; the [sample config](https://github.com/ARTPARK-SAHAI-ORG/arcval/blob/main/examples/tts/config.json) uses `google/gemini-2.5-flash`. |
...
Refer to the [sample config](https://github.com/ARTPARK-SAHAI-ORG/arcval/blob/main/examples/tts/config.json) for a template.
...
<AccordionGroup>
  <Accordion title="pronunciation (default evaluator system prompt)">
    Matches [`DEFAULT_TTS_EVALUATOR`](https://github.com/ARTPARK-SAHAI-ORG/arcval/blob/main/arcval/judges.py) in `arcval/judges.py` when no `--config` is passed.
